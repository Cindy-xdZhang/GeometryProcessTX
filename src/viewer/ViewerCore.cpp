...
void ViewerCore::align_camera_center(const Eigen::MatrixXd& V)
{
    if (V.rows() == 0)
    {
        return;
...
void ViewerCore::align_camera_center(const Eigen::MatrixXd& V, const Eigen::MatrixXi& F)
{
    if (V.rows() == 0)
    {
        return;
        ...
        Eigen::Vector3f& shift)
{
    ...
    zoom = 2.0f / (max_point - min_point).array().abs().maxCoeff();
}
...
void ViewerCore::draw(ViewerData& data, bool update_matrices)
{
    ...
    if (update_matrices)
    {
        ...
        // Set view
        utils::look_at(camera_eye, camera_center, camera_up, view);
        view = view
        ...
        if (orthographic)
        {
            ...
            float h = tan(camera_view_angle / 360.0 * utils::PI) * (length);
            utils::ortho(-h * width / height, h * width / height, -h, h, camera_dnear, camera_dfar, proj);
        }
        else
        {
            float fH = tan(camera_view_angle / 360.0 * utils::PI) * camera_dnear;
            float fW = fH * (double) width / (double) height;
            utils::frustum(-fW, fW, -fH, fH, camera_dnear, camera_dfar, proj);
        }
    }
...
void ViewerCore::draw_buffer(ViewerData& data,
...
void ViewerCore::draw_labels(ViewerData& data, const MeshGL::TextGL& labels)
{
...
void ViewerCore::set_rotation_type(
        const ViewerCore::RotationType& value)
{
    using namespace Eigen;
    using namespace std;
    ...
    {
        utils::snap_to_fixed_up(Quaternionf(trackball_angle), trackball_angle);
    }
}

void ViewerCore::set(unsigned int& property_mask, bool value) const
{
    if (!value)
    {
        unset(property_mask);
...
void ViewerCore::unset(unsigned int& property_mask) const
{
    property_mask &= ~id;
}

void ViewerCore::toggle(unsigned int& property_mask) const
{
    property_mask ^= id;
}

bool ViewerCore::is_set(unsigned int property_mask) const
{

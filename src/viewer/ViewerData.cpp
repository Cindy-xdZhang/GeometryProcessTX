        ...
        const Eigen::MatrixXd& _V, const Eigen::MatrixXi& _F)
{
    ...
    if (V.rows() == 0 && F.rows() == 0)
    {
        ...
        compute_normals();
        uniform_colors(
                Eigen::Vector3d(color::GOLD_AMBIENT[0], color::GOLD_AMBIENT[1], color::GOLD_AMBIENT[2]),
                Eigen::Vector3d(color::GOLD_DIFFUSE[0], color::GOLD_DIFFUSE[1], color::GOLD_DIFFUSE[2]),
                Eigen::Vector3d(color::GOLD_SPECULAR[0], color::GOLD_SPECULAR[1], color::GOLD_SPECULAR[2]));
    }
    else
    {
        ...
        {
            cerr
                    << "ERROR (set_mesh): The new mesh has a different number of vertices/faces. Please clear the mesh before plotting."
                    << endl;
        }
    }
...
void ViewerData::set_mesh(
        const Eigen::MatrixXd& _V, const Eigen::MatrixXi& _F, const Eigen::MatrixXi& _FtoF)
{
    set_mesh(_V, _F);
    FtoF = _FtoF;
}
...
void ViewerData::set_visible(bool value, unsigned int core_id /*= 1*/)
{
    if (value)
    {
        is_visible |= core_id;
...
void ViewerData::copy_options(const ViewerCore& from, const ViewerCore& to)
{
...
void ViewerData::set_data(
        ...
        double caxis_min,
        double caxis_max,
        colormap::ColorMapType cmap,
        int num_steps)
{
    if (!show_texture)
    {
        Eigen::MatrixXd CM;
        colormap::colormap(cmap, Eigen::VectorXd::LinSpaced(num_steps, 0, 1).eval(), 0, 1, CM);
        set_colormap(CM);
    }
    set_uv(((D.array() - caxis_min) / (caxis_max - caxis_min)).replicate(1, 2));
}

void ViewerData::set_data(const Eigen::VectorXd& D, colormap::ColorMapType cmap, int num_steps)
{
...
void ViewerData::empty_texture(unsigned int size)
{
    texture_R = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(size, size, 255);
    texture_G = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(size, size, 255);
    texture_B = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(size, size, 255);
    texture_A = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(size, size, 255);
...
void ViewerData::grid_texture()
{
    ...
    texture_G = texture_R;
    texture_B = texture_R;
    texture_A = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(texture_R.rows(),
                                                                                       texture_R.cols(), 255);
    dirty |= MeshGL::DIRTY_TEXTURE;
}

void ViewerData::clear_texture()
{
    unsigned x_size = texture_R.rows();
    unsigned y_size = texture_R.cols();

    texture_R = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(x_size, y_size, 255);
    texture_G = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(x_size, y_size, 255);
    texture_B = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(x_size, y_size, 255);
    texture_A = Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>::Constant(x_size, y_size, 255);
    ...
    else
    {
        ...
        if ((meshgl.dirty & MeshGL::DIRTY_UV) && data.V_uv.rows() > 0)
        {
            ...
            for (unsigned i = 0; i < data.F.rows(); ++i)
            {
                for (unsigned j = 0; j < 3; ++j)
                {
                    meshgl.V_uv_vbo.row(i * 3 + j) = data.V_uv.row(
                            per_corner_uv ? data.F_uv(i, j) : data.F(i, j)).cast<float>();
                }
            }
        }
    }
